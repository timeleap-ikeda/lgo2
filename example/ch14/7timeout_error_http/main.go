...
func main() {
	ctx, cancelFuncParent := context.WithTimeout(context.Background(), 3*time.Second)  //liststart1
...
loop:
	for {
		...
		case <-ctx.Done():
			fmt.Println("in main: cancelled with cause:", context.Cause(ctx), "err:", ctx.Err())  //liststart2
			break loop
		}
	}
	wg.Wait()
